...
| 遮蔽对象 | 意义、关系、因果、世界状态和语义主权。 |
| 内在残差 | 空间越可度量，越暴露出意义不在空间里。 |
...
阶段 II 的成功标准是空间场正确：像素、区域、实例、边界都能被度量化，视觉终于不再只在整图标签上闭合。

但这个成功反过来暴露出它自己的盲区：

> 稠密度量的胜利，产生了语义空洞。

也就是说，空间场可以被准确分割，却未必知道自己分割的是什么；边界可以很准，实例可以很稳，但“这个对象为何重要、属于什么开放概念、能否被语言调用”仍然不在阶段 II 自己的闭合里。阶段 III 不是外部加语言，而是阶段 II 的稠密成功逼出的公共语义闭合。

更完整地说，阶段 II 有三层自我否定：

| 层级 | 成功 | 自我否定 | 推出 |
|---|---|---|---|
| 场闭合 | 每个像素/位置都有答案 | 像素不是对象；局部正确不等于实体成立 | 对象闭合 |
| 对象闭合 | 每个实例可被分离 | 对象依赖 anchor、NMS、RoI、assignment、query 等绑定制度 | 接口闭合 |
| 接口闭合 | 外部 prompt 可调用区域/对象 | 语义仍由语言、示例、概念数据引擎赋予 | 阶段 III 公共语义闭合 |

一句话压缩：**空间越可度量，越暴露出意义不在空间里。**
...
> 语义分割、实例分割、全景分割可以共享同一种 mask classification 架构和大量训练机制——预测 N 个 (mask, class) 对。
> 

Mask2Former 把三个分割任务统一到**同一套 query-based 架构**，OneFormer 再加一个任务 token 让同一套权重跑三个任务，EoMT 把 query 直接嵌进 ViT encoder，说明强预训练 ViT 可以在部分分割任务上承担大量 decoder 工作。但同一架构统一不等于三者本体完全同一：语义、实例、全景仍保留不同评价目标和错误类型。
...
**共同结构**：多数阶段 II 开集识别仍没有完全自持的视觉概念空间，常见机制是「**用文本/语言的概念，去筛选视觉的定位候选**」。

> 视觉模型学的是「**哪里**」，语义靠语言模型外挂。
> 

这和 NLP 的演化不完全对称。NLP 模型从 word2vec → BERT → GPT 一路在**构建语言内部的语义空间**。视觉也有 DINO/JEPA/MAE 等非语言路线，但阶段 II 的开放词汇主流仍高度依赖 CLIP 式图文对齐；CLIP 的「视觉空间」很大程度上是**被语言拉对齐**的投影，而不是完全独立生长的概念制度。
...
| 展开环节 | 技术对象 | 理念功能 | 自我否定 |
|---|---|---|---|
| 像素化 | FCN、U-Net、SegNet | 度量进入图像定义域 | 像素不是对象 |
| 上下文化 | DeepLab、ASPP、PSPNet、HRNet、SegFormer | 每个位置获得远处上下文 | 全局上下文不等于对象关系 |
| 对象化 | R-CNN、YOLO、SSD、FPN、Mask R-CNN | 连续图像场中出现可数实体 | 依赖 anchor、NMS、RoI、assignment |
| 集合化 | DETR、Deformable、DAB、DINO、Co-DETR | 对象成为 set element | query 自身成为黑箱 |
| mask 统一 | MaskFormer、Mask2Former、OneFormer、EoMT | 语义/实例/全景共享 mask 原语 | 闭集类别仍锁死语义 |
| 提示化 | SAM、SegGPT、Grounding DINO | 外部 prompt 可调用稠密区域 | 语义来自外部，不属于视觉自身 |
| 开放词汇 | SAM 3、Florence-2、YOLO-World、CAT-Seg、Depth Anything | 类别/任务/几何进入开放接口 | 关系、因果、世界状态仍未闭合 |
| 自我否定 | VLM 看不清、MMVP、visual-unnecessary answering | 语言补语义，但遮蔽视觉 | 阶段 III：公共语义闭合必须真正内化 |
...
**阶段二的条件性公理**：阶段二后期的大量区域、对象、实例、开放词汇任务，可以被表达为「query / prompt 在 dense feature field 上读取相容区域，并输出位置-语义对」。差别只在：
...
这四个槽位的组合，解释了阶段二晚期大量区域/对象类模型。但连续场任务、物理量估计、低层恢复、时序预测和 3D/4D 状态任务，不必全部落入这个形式。

</aside>

这条条件性公理**解释**了为什么 DETR、Mask2Former、SAM、Grounding DINO、Florence-2、APE 看起来差别很大，实际上常常只是**四个槽位的不同填充**；也给阶段二内部的新模型提供了一条判定线：它究竟只是重新填槽，还是改变了 field、object、interface、metric 中的某个结构。

真正的下一次跃迁，来自这条公理暴露出的残差：语义来源、视觉粒度、关系结构、度量反身和世界状态不再能被单一 query-readout 协议充分解释。

## 八、阶段 II 出口的三条后续路线

阶段 II 的出口不是单一路径。公共语义路线进入阶段 III，视觉自持路线进入阶段 V，工艺聚合路线则成为跨阶段基础设施。

### 路线 A：公共语义路线 -> 阶段 III

核心问题是：投影表面已经能被点、框、mask、文本和示例调用，但这些输出如何进入公共意义？

Grounding DINO、Florence-2、LLaVA、Qwen-VL、LISA / GLaMM 等路线把阶段 II 的空间接口接到语言契约上。它们不是简单“加语言”，而是在回答阶段 II 的语义空洞。

### 路线 B：视觉自持路线 -> 阶段 V

JEPA / DINOv3 / Fluid / V-JEPA 2 等路线试图**让视觉模型不依赖语言也能学到语义结构或状态结构**。JEPA 系列用「在嵌入空间预测」代替 CLIP 的「图-文对齐」，DINOv3 的 Gram Anchoring 保护局部几何，Fluid 证明「无词汇视觉 scaling」可以比 CLIP 学得更好。

**核心赌注**：视觉有自己的视觉结构和状态变量，只是之前没有被正确的目标函数激活。

### 路线 C：推理与工艺聚合路线 -> 横切基础设施

测试时视觉回看（Visual CoT / TTSP / MVoT）接受「一次性前向 pass 看不清」这个事实，改用**推理时多次回看**来补偿。聚合蒸馏（RADIO / C-RADIOv4 / MoF）则不赌单个范式能自我救赎，而是把 CLIP、DINOv2、SAM、MAE 的互补知识蒸进统一 backbone。

**核心赌注**：没有唯一最好的 backbone；阶段 II 末端的工程现实，是多种闭合制度的互补能力被聚合为基础设施。
