...
这一段的关键在于“判别方向如何变成形状”。下面用具体数值例子展开。
...
回到核心问题："cross-attention 是异构场之间的共形桥，那 projector 承担了什么角色？"
...
并非所有 VLM 都不用 cross-attention，例如**Flamingo (DeepMind, 2022)** 就用了 cross-attention：
...
这是一个很锋利的问题。这个看似“退步”的 VLM 架构选择，实际上是**一个经过深思熟虑的工程权衡**。
...
$$
P_k = \text{Conv}_{1\times1}\big(\text{Concat}(\text{Lateral}(C_k), \text{Upsample}(P_{k+1}))\big)
$$

**Concat + 1×1 conv 版本（concat 候选）**：
...
- 类比：若要问"哪个餐厅最近"，精度要求在于"问这个问题的地点"（Q），而不在于"全市的餐厅索引"（K/V）——后者可以被压缩成 coarse lookup
...
继续展开。这两篇都属于"架构成熟之后的清算型工作"——一个清算稠密 decoder，一个清算"每任务一份权重"的残余。
