...
**所以 FPN 不是瓶颈层**。它不是"压缩到一个最小表征再展开"——它是**多尺度特征的横向增强器**：每一尺度的 feature 都被"更高层的语义"补齐。可将它理解为"语义在多尺度上的等势面分布"来理解更准确。
...
这个理解成立。**通道 = 视觉里的 embedding size** 这个等式是成立的；但理解为"embedding"还是"feature channel"，决定了后续操作的形状——这是一个**心智模型层面的区分**，不是数学层面的区分。

---

# §4 · 分割里 C 维向量 → per-pixel 分类 = "隐式 query"

这一点是 MaskFormer 范式的关键。把它形式化之后，可以看到 FCN 分类头与 mask query 之间的结构连续性。
...
**核心判断**：**FCN 的分类头就是 K 个隐式 query**。每个 class prototype 是一个"查询向量"，查询某个像素是否属于对应类别。
...
原表述里混了"prototype 数"和"prototype 维度"（原表述中的 C 其实指 prototype 数），但核心洞察完全对。
