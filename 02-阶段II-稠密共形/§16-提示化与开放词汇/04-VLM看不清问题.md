...
**合起来的结论**：许多当前主流 VLM 在困难视觉判断上表现出一种结构性风险：视觉输入先被压缩成语言先验容易接管的粗粒度线索，随后主要在语言空间中完成推理。这不是所有 VLM 的本质定义，而是当前架构与训练制度下反复暴露的倾向。
...
这页在阶段 II 中应被读成一条关键否定经验：

1. 第一层否定：空间场可以很准，但没有公共语义。
2. 第二层否定：引入语言后，语义似乎补上了；但语言可能遮蔽视觉，使模型说得更像懂，却看得并不更清楚。
3. 第三层否定：阶段 III 的任务不是简单“给视觉接上语言”，而是解决语言与视觉之间的真实共形，避免语言对视觉的替代。

因此，VLM 看不清不是阶段 II 的边角问题，而是阶段 II 出口残差在阶段 III 入口处的量化显影：**语言补语义，也可能遮蔽视觉。**
...
这一段与前文的 scaling 分析可以共同构成一份完整的**阶段 II → 阶段 III 过渡总纲**：前者解释视觉自有 scaling 的上限，后者解释借来语义的代价，合起来才构成阶段 III 的入口宣言。就结构而言，它们应被视作同一组跨阶段论证的上下两半。
