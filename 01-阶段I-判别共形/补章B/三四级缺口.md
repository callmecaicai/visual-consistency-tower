...
- 在 ImageNet 级别通用视觉和现代 scaling 条件下，「Equivariance 应硬编码」的 capsule 路线没有赢过「数据增强 + self-attention 的学出来」路线。

### 本质教训

- **架构先验的正确性 ≠ 可训练性 + 可 scale 性**。
- 这条教训直接为 **ViT 的成功**铺路:ViT 减弱了 CNN 的强局部空间先验,但并非没有归纳偏置；patch 化、位置编码、LayerNorm、训练 recipe 和数据增强仍在塑形。强手工先验在小数据时带来效率,在大数据时可能带来约束。
- 与 NAS 的教训同构:在通用大规模视觉上，软式假设 + 规模经常胜过硬式假设 + 复杂约束；但在小数据、医学、3D、物理场、群等变任务中，硬先验仍可能有效。
...
3. **Softmax 的条件唯一性**:在 K 类分布的 sufficient statistics 假设下,softmax 是 **exponential family** 的**自然参数化** — 即 Boltzmann 分布。它不是任意选择,但这个结论依赖单标签分类与指数族参数化条件。
...
**softmax + CE 不是随意工程选择,而是「最大似然 + exponential family + proper scoring rule」三重约束下的条件交集。**
