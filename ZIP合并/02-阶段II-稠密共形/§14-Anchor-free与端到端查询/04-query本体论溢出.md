...
要不要把这段（§1-§8）作为"**object query 作为通用接口原语**"的独立一节，写进 [阶段 II · Anchor-free 与端到端查询：从先验到查询（CornerNet / CenterNet / FCOS / DETR / Deformable DETR / DINO-DETR / Co-DETR）](00-定位卡.md) 页面的末尾（在 DETR 四代深度展开之后，作为 §6.Deta 后面的 §7）？或者**更合适的做法**是把它作为**阶段 II → 阶段 III 的桥梁**单独开一页——因为这个原语实际上是跨阶段的本体论，塞在阶段 II 的一个子页里会被低估。两个选项：
