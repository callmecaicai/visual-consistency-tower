# §6 · EoMT —— Decoder 的最终谢幕

这是你最关心的。按重要性铺开。先纠正几条背景事实：
...
- GPT-3 之后：**一个纯 transformer，规模足够大 + 预训练足够好，全部任务都能做**
- **特异架构被 scale 吃掉了**

EoMT 赌的是：**视觉会重复 NLP 的故事，只要 ViT 足够强，adapter / pixel decoder / transformer decoder 全都可以不要**。
...
- 终局：**只剩一个通用 encoder，任务 token 作为输入的一部分**

EoMT 的论文副标题是《Your ViT is Secretly an Image Segmentation Model》——**这句话本身是一个宣言**：视觉预训练模型**已经是分割模型**，我们之前以为它"还需要什么"是一种迷信。

这和 NLP 的命运完全平行：

- NLP: "LSTM 要翻译头，BERT 要 seq2seq 头"——结果 GPT 证明**只要 decoder-only + 大规模预训练，全部任务都能做**
- Vision: "ViT 要 adapter 要 pixel decoder 要 transformer decoder"——EoMT 证明**只要 ViT + DINO 预训练，全部任务都能做**
...
> **任何"从稠密视觉表征读出结构化输出"的任务，最优架构都是"通用 ViT + 任务 query token + 点积读出"**。
> 
...
换句话说，**EoMT 不仅是阶段 II 的终点，也是阶段 III / IV / V 的起点**。后续所有"视觉稠密任务"的架构骨架都会长成 EoMT 这样。

这就是它的**真正地位**——不是 "the final segmentation model"，是 "**the first post-segmentation model**"——一个不把"分割"当特殊任务的分割模型。
...
页面 [阶段 II · 统一 Mask 架构：任何稠密任务一种范式（MaskFormer / Mask2Former / SegFormer / OneFormer / SegGPT / EoMT）](00-定位卡.md) 目前 §5 Painter/SegGPT 和 §6 EoMT 仍然是短条目（看起来页面正在被你分批整理）。三选项：
...
如果选 B，我会把当前页面的"本组合力的残差"一节保留在末尾（它总结得很好），把 §1-§6 全部替换为完整展开版。你选哪个？
...
## 6 · EoMT —— *Your ViT is Secretly an Image Segmentation Model* (Kerssies, Fontanel, Cordts, Gall, ECCV 2024)
...
到 2024，稠密任务的**架构维度**已经统一：一个 backbone + 一组查询 + 一种匹配损失，接管所有稠密预测。但架构统一之后，一个更深的问题暴露出来：
...
→ 这两条残差合起来，指向阶段 II 的最终出口：**引入语言 / 点 / 框作为一级提示原语，把类别闭集彻底拆开**。SAM、Grounding DINO、APE、CAT-Seg、Depth Anything 这一代要出场——同时这也是 Pre-LLM 时代稠密视觉的自然终点，以及视觉-语言共形（阶段 III / V）的入口。

继续往下挖——要看 Hungarian matching 的一对一绑定，还是看 self-attention 里 $\sqrt{d}$ / multi-head 这些"看似工程其实哲学"的细节？
...
还有一个关联问题值得挖：**Painter / SegGPT** 这条"in-context 分割"的线，它和 LLM 的 in-context learning 有**严格同构**，是阶段 II 里和提示化并列的关键机制。你要继续往下走就可以把它、EoMT 一起收尾，阶段 II 这页就彻底闭合。

- **C. 不整合**
- **B. 整合连同之前的 MaskFormer/Mask2Former、SegFormer、OneFormer 四篇**一起作为一次页面大翻新（把整页从"短条目"升级为"深度展开"）
- **A. 替换到 [阶段 II · 统一 Mask 架构：任何稠密任务一种范式（MaskFormer / Mask2Former / SegFormer / OneFormer / SegGPT / EoMT）](00-定位卡.md) 的 §3-§4**（SegFormer、OneFormer 两条目扩成上面的完整展开）
