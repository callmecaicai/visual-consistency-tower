...
要不要把这段（§1-§8）作为"**object query 作为通用接口原语**"的独立一节，写进 [阶段 II · Anchor-free 与端到端查询：从先验到查询（CornerNet / CenterNet / FCOS / DETR / Deformable DETR / DINO-DETR / Co-DETR）](..\§14-Anchor-free与端到端查询\00-定位卡.md) 页面的末尾（在 DETR 四代深度展开之后，作为 §6.Deta 后面的 §7）？或者**更合适的做法**是把它作为**阶段 II → 阶段 III 的桥梁**单独开一页——因为这个原语实际上是跨阶段的本体论，塞在阶段 II 的一个子页里会被低估。两个选项：

- **A. 塞进当前页**（作为收尾一节）
- **B. 开新页**：命名类似「阶段 II → III 的桥梁：object query 作为通用接口原语」，作为序号 14.5 或阶段 II 和 III 之间的过渡页
- **C. 不整合**
...
[阶段 II · 统一 Mask 架构：任何稠密任务一种范式（MaskFormer / Mask2Former / SegFormer / OneFormer / SegGPT / EoMT）](00-定位卡.md) 这一页现在只有 MaskFormer / Mask2Former 两条短条目，然后直接跳到 SegFormer。上面这份展开可以作为 §1–§2 的完整版直接替换，保持和你前面 DETR 线、YOLO 线同样的密度。三个选项（和前两次一样）:

- **A. 直接替换**：把 1、2 条目换成上面的完整展开（§0 到 §5），页面结构上约占 80%——因为这两篇是本页的核心，展开份量应最大
- **B. 插在条目之间**：保留原有短描述作为速览，在 6 条目之后加一节「MaskFormer / Mask2Former 深度展开」
- **C. 不整合**：只留聊天
