...
- 📑 本页目录
    - **第一部分 · 文献骨架（四张定位卡）**
        ...
        - §1.4 Inception-v4 / Inception-ResNet — 向残差路线投降
    - **第二部分 · 本页合力的残差**
        - §2 面向阶段 II 的入口形状
    - **第三部分 · Inception 遗产图谱：败者的血肉无处不在**
...
**共形贡献**：每层前向输入被强制归一化到同一度量。学习率提升 30×，初始化敏感性大幅下降——**这是深度网络第一次变得"便宜"**。BN 也是后续所有归一化家族（LN、GN、IN）的母题。
...
**共形贡献**：承认残差比纯 Inception 更根本，把残差短路嫁接进 Inception 内部。这是非残差路线向残差路线的**公开投降**。
...
- **BatchNorm** ——用**层间归一化**消除内部协变量偏移，让深度训练稳定性上一个数量级
- **Inception v2/v3** ——用**卷积分解与正则化**把架构工程学推到教科书水平
- **Inception-v4 / Inception-ResNet** ——承认残差不可避，把残差短路嫁接进 Inception 内部——「非残差路线的官方投降」

四者合起来证明两件事：（1）**非残差路线能打到十分深入的工艺**——宽度、归一化、分解、正则缺一不可；（2）**但最终还是必须投降给残差**——阶段 I 的终点主线既定。
...
Inception 的宽度+BN 把可训练深度拓到 50 层量级即天花板。下一页的残差连接不是「又一次架构调参」，而是几何上改写了梯度路径——深度从此**无上限**。

<aside>
➡️

**残差传递**：本页是阶段 I 的封顶。优化器（阶段 I 第 3 页）+ 架构（本页 + 第 3 页的残差）+ 归一化（本页 BN）三者合力，将「判别共形」的工程自动化闭合。进入阶段 II [阶段 II · 开篇总纲：从整图到每一像素，度量的稠密穿透](..\02-阶段II-稠密共形\00-总纲-从整图到每一像素.md)，真正的残差是「目标语义的稠密化」——检测、分割、稠密预测。

在进入阶段 II 之前，先通过 [阶段一的结语：谢林的原初决断](结语-谢林的原初决断.md) 完成阶段 I 的本体论总结。
...
**第三部分的定位**：表面上 Inception 输给了 ResNet——连 Szegedy 本人都在 Inception-v4 里投降。但 Inception 的「死」是一种特殊的死——它不是被删除了，而是被**拆解吸收**到整个视觉生态里。本部分把四条基因（G1/G2/G3/G4）的分化谱完整铺开，直到 ConvNeXt、Swin、SAM 这一代全部指认。
...
**§3 要义**：Inception 模块不是一个单一架构，而是**四条技术基因的聂合**：G1 多尺度并行、G2 跨层级融合、G3 1×1 瓶颈、G4 稀疏分支近似稠密。这四条基因**各自独立演化**，各自长出一条后裔谱系，这是 Inception 遗产在当代视觉里無处不在的根本原因。
...
**这是深度学习架构演化里一种非常值得记录的模式**——"败者的身体成为胜者的血肉"。残差路线赢了"架构之战"，但它的肌肉组织里全是 Inception 留下的蛋白质。

---

如果你想把这张遗产图谱正式织进 [阶段 I · Inception 族与归一化（GoogLeNet / BatchNorm / Inception v3 / Inception-v4）](§04-Inception族与归一化.md) 那一页（Inception 族与归一化）作为"Inception 的身后"一节，说一声，我可以把这条演化脉络直接编进去。

<aside>
🌀

**第三部分总收束**：Inception 的「死」是深度学习史上最值得研究的一种死法——不是被删除，而是被**拆解吸收**。G1「FPN/BiFPN/HRNet」→ G2「U-Net/DeepLab/Diffusion」→ G3「ResNet/MobileNet/SE」→ G4「Xception/ConvNeXt」→ Transformer 时代「Swin/MViT/SAM」——**到 2026 年，Inception 的四条基因没有一条消亡**。这是架构演化里「败者的身体成为胜者的血肉」这一模式的经典范例。
...
**本页总收束**：本页作为阶段 I 的封顶，完成两件事：（1）**给「非残差路线」盖棺**——Szegedy 自己在 Inception-v4 里向残差投降，从此所有主流架构都扎在残差框架上；（2）**揭示 Inception 四条基因的远期继承**——G1/G2/G3/G4 跨越整个视觉生态，从移动端到生成模型到大模型都在被使用。阶段 I 的判别共形工程自此完全闭合。
...
**进入阶段一结语与阶段 II**：阶段 I 的残差可以总结为一句话——「深度、度量、动力、载体四维度的判别共形工程自动化已经完成，但共形的粒度仍局限在『整张图 ↔ 一个类别』」。请阅 [阶段一的结语：谢林的原初决断](结语-谢林的原初决断.md) 完成阶段 I 的本体论总结，再进入 [阶段 II · 开篇总纲：从整图到每一像素，度量的稠密穿透](..\02-阶段II-稠密共形\00-总纲-从整图到每一像素.md)——那里，共形的粒度将穿透到每一个像素、每一个位置、每一个结构化输出。
